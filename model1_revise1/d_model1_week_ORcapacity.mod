...
// Surgery types (indexed by integer codes, e.g., 1..10)
{int} SurgeryTypes = ...;

// [NEW] Operating rooms (e.g., {1,2})
{int} K = ...;
...
// [NEW] OR turnover / cleaning time by type (minutes)
...
// ------------------------------
// [NEW] Operating Room capacity variables
// ------------------------------
...
forall(s in S, p in P, d in D) {
  ...
  // (9) & (10) Define fssp: 1 iff both surgeons s and s2 attend patient p on day d
  ...
  /***** 5. DAILY WORKING HOURS & START TIME CLEANUP *****/
  ...
  // Cleanup: if surgeon s does not operate p on day d, force start time = 0
  forall(s in S, p in P, d in D)
...
execute {
   var o = new IloOplOutputFile("phase1_results.dat");
   ...
   // Export wsp (main/responsible surgeon)
   o.writeln("wsp_in = [");
   sCount = 0;
   for(var s in S) {
      o.write("  [");
      ...
      for(var p in P) {
         ...
         var dCount = 0;
         for(var d in D) {
            if (wsp[s][p][d] >= 0.99) o.write("1"); else o.write("0");
            dCount++;
            if (dCount < numD) o.write(", ");
         }
         o.write("]");
   ...
   for(var s in S) {
      ...
      for(var p in P) {
         ...
         var dCount = 0;
         for(var d in D) {
            if (ysp[s][p][d] >= 0.99) o.write("1"); else o.write("0");
            dCount++;
            if (dCount < numD) o.write(", ");
         }
         o.write("]");
   ...
   for(var s in S) {
      ...
      for(var p in P) {
         ...
         var dCount = 0;
         for(var d in D) {
            if (zsp[s][p][d] >= 0.99) o.write("1"); else o.write("0");
            dCount++;
            if (dCount < numD) o.write(", ");
         }
         o.write("]");
   ...
   o.close();
   writeln(">>> Exported phase1_results.dat successfully (with team roles) <<<");
}