...
def run_online_simulation(
    ...
    for idx, (urgent_arrival_time, urgent_type) in enumerate(sorted(urgent_list)):
        frozen_time = int(urgent_arrival_time)
    ...
    # Calculate percentage improvement vs initial baseline
    if all_metrics:
        initial_baseline = all_metrics[0]["baseline_fitness"]
        
        # Find last valid best_fitness (non-zero, from actual GA run)
        final_best = initial_baseline  # Default
        for m in reversed(all_metrics):
            if m["best_fitness"] > 0:
                final_best = m["best_fitness"]
                break
        
        pct_improvement = ((initial_baseline - final_best) / initial_baseline * 100) if initial_baseline > 0 else 0
        print(f"Initial baseline fitness: {initial_baseline:.1f}")
        print(f"Final best fitness: {final_best:.1f}")
        print(f"Total improvement: {total_improvement:.1f} ({pct_improvement:.2f}%)")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--initial_schedule", type=str, 
                       default=os.path.join(_SCRIPT_DIR, "large_rulebased_output.xlsx"),
                       help="Path to initial elective schedule")
    parser.add_argument("--work_schedule", type=str,
                       default=os.path.join(_SCRIPT_DIR, "lich_lam_viec_tuan1_large.xlsx"))
