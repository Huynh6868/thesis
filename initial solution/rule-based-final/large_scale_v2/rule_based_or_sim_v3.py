...
# ASSIST2_POOL = {f"S{i}" for i in range(9, 21)}  # S9..S20 for large scale

# Mapping from short names (heuristic output) to full names (for SURGERY_DURATION_MIN lookup)
...
def normalize_surgery_type(stype: str) -> str:
    """Convert short surgery type name to full name for duration/prep lookup."""
    # First check if it's already a full name (lowercase match)
    stype_lower = stype.lower().strip()
    for full in SURGERY_DURATION_MIN.keys():
        if stype_lower == full.lower():
            return full
    # Check short name mapping
    for short, full in SHORT_TO_FULL_SURGERY_TYPE.items():
        if stype_lower == short.lower():
            return full
    # Return original if no mapping found (will likely cause error later)
    return stype
...
def load_cap_rank_xlsx(path: str, capability_sheet: str = "Capabilities") -> CapabilityModel:
    """
    Load capability and ranking data from Excel.
    
    Args:
        path: Path to Cap_Rank.xlsx
        capability_sheet: Name of capability sheet ('Capabilities', 'capabilities med', 'capabilities large')
    
    Returns:
        CapabilityModel with surgeon capabilities and surgery rankings
    """
...
def load_room_config(cap_rank_path: str, scale: str = 'large') -> int:
    """
    ...
    try:
        ...
        # Fallback: try 'num_rooms' column
        if 'num_rooms' in df.columns:
            return int(df['num_rooms'].iloc[0])
        # Last resort: large is row 2 by default
        return int(df.iloc[2, 1]) if scale == 'large' else 2
    except Exception as e:
...
def load_elective_schedule_xlsx(path: str, return_df: bool = False):
    ...
    for _, row in df.iterrows():
        ...
        stype_raw = str(row["surgery_type"]).strip()
...
# ------------------------------------------------------------
def run_sim(
    work_schedule_paths: List[str],  # CHANGED: Now accepts list of paths for multi-week
    cap_rank_path: str,
    ...
    seed: int,
    capability_sheet: str = "Capabilities",  # Scale-specific capability sheet
    surgeons_list: List[str] = None,  # Scale-specific surgeon list
):
    random.seed(seed)

    # ---- Load inputs
    # Load work schedules for all weeks
    df_weeks = []
    for path in work_schedule_paths:
        df_weeks.append(load_work_schedule_xlsx(path))
    
    # Use provided surgeon list or default to S1-S12
    if surgeons_list is None:
        surgeons = [f"S{i}" for i in range(1, 13)]  # S1..S12 (default small scale)
    else:
        surgeons = surgeons_list
    
    # Create WorkSchedule with list of DataFrames (multi-week support)
    work = WorkSchedule(df_weeks, surgeons)

    # Load capabilities from scale-specific sheet
    cap = load_cap_rank_xlsx(cap_rank_path, capability_sheet=capability_sheet)
    ...
    # Load room config from Excel (instead of determining from schedule)
    n_rooms = load_room_config(cap_rank_path)
...
def main():
    ...
    try:
        import scale_config
        SCALE = 'large'  # LARGE SCALE configuration
        config = scale_config.get_scale_config(SCALE)
        surgeons_list = scale_config.get_surgeon_list(SCALE)
        print(f"\n=== LARGE SCALE SIMULATION ({config['num_doctors']} surgeons) ===\n")
    except ImportError:
        print("Warning: scale_config not found, using defaults")
        config = {'patient_file': 'large_scale_result.xlsx', 'capability_sheet': 'capabilities large', 
                  'work_schedules': ['lich_lam_viec_tuan1_large.xlsx', 'lich_lam_viec_tuan2_large.xlsx'], 'num_doctors': 20}
        surgeons_list = [f"S{i}" for i in range(1, 21)]  # 20 surgeons
        SCALE = 'large'
    
    # Default file paths using scale config
    default_patient_file = os.path.join(script_dir, config['patient_file'])
    default_work_schedule = os.path.join(script_dir, config['work_schedules'][0])  # Use first week
    
    parser = argparse.ArgumentParser()
    parser.add_argument("--work_schedule", default=default_work_schedule, 
                       help="Weekly doctor work schedule (.xlsx)")
    parser.add_argument("--cap_rank", default=os.path.join(script_dir, "Cap_Rank.xlsx"), 
                       help="Capabilities + ranking (.xlsx)")
    parser.add_argument("--elective_sched", default=default_patient_file, 
                       help="Fixed elective schedule (.xlsx)")
    parser.add_argument("--mean_urgent", type=float, default=None, 
                       help="Mean inter-arrival time for urgent (minutes)")
    parser.add_argument("--rest_time", type=int, default=DEFAULT_REST_TIME_MIN, 
                       help="Surgeon rest time after surgery (minutes)")
    parser.add_argument("--penalty_next_week", type=int, default=DEFAULT_PENALTY_DELAY_NEXT_WEEK, 
                       help="Penalty per elective pushed to next week")
    parser.add_argument("--seed", type=int, default=1)
    parser.add_argument("--output", type=str, default=None, 
                       help="Output path for schedule Excel file")
    
    args = parser.parse_args()
    
    # Load urgent parameter from Excel based on scale
    if args.mean_urgent is None:
        scale_name = 'Large scale' if SCALE == 'large' else ('Medium scale' if SCALE == 'medium' else 'Small scale')
        args.mean_urgent = load_urgent_param_from_excel(args.cap_rank, scale_name)
    print(f"Using mean_interarrival_urgent: {args.mean_urgent} ({SCALE} scale)")
    
    # Load capability sheet based on scale
    print(f"Loading capabilities from sheet: '{config['capability_sheet']}'")
    
    # Load room config
    try:
        num_rooms = load_room_config(args.cap_rank, scale=SCALE)  # Pass scale for correct room count
    ...
    try:
        ...
        print(f"Loaded rest time configuration for {len(rest_time_map)} surgery types")
    except Exception as e:
        print(f"Warning: Could not load rest time map: {e}")
        rest_time_map = None
    
    # Build work schedule paths list from config
    work_schedule_paths = []
    for schedule_file in config.get('work_schedules', ['lich_lam_viec_tuan1.xlsx']):
        work_schedule_paths.append(os.path.join(script_dir, schedule_file))
    print(f"Loading {len(work_schedule_paths)} week(s) of work schedules")

    log, stats, surgeon_kpi = run_sim(
        work_schedule_paths=work_schedule_paths,  # CHANGED: Now passing list
        ...
        seed=args.seed,
        capability_sheet=config.get('capability_sheet', 'Capabilities'),
        surgeons_list=surgeons_list,
    )
    
    # Export schedule - auto-export by default, or to custom path if specified
    default_output = os.path.join(script_dir, "large_rulebased_output.xlsx")
