...
# ASSIST2_POOL = {f"S{i}" for i in range(9, 17)}  # S9..S16 for medium scale
...
def load_room_config(cap_rank_path: str, scale: str = 'medium') -> int:
    """
    ...
    try:
        ...
        # Last resort: medium is row 1 by default
        return int(df.iloc[1, 1]) if scale == 'medium' else 2
    except Exception as e:
...
def main():
    ...
    try:
        ...
        print(f"\n=== MEDIUM SCALE SIMULATION ({config['num_doctors']} surgeons) ===\n")
    except ImportError:
        print("Warning: scale_config not found, using defaults")
        config = {'patient_file': 'surgery_schedule.xlsx', 'capability_sheet': 'Capabilities', 
                  'work_schedules': ['lich_lam_viec_tuan1.xlsx'], 'num_doctors': 12}
        surgeons_list = [f"S{i}" for i in range(1, 13)]
        SCALE = 'small'
    ...
    if args.mean_urgent is None:
        scale_name = 'Medium scale' if SCALE == 'medium' else 'Small scale'
