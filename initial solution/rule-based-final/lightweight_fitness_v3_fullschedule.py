...
# ============================================================
# Added helper: evaluate a FIXED FULL schedule (no dispatch)
# ============================================================
def compute_metrics_from_fixed_schedule_df(
    schedule_df,
    work: sim.WorkSchedule,
    cap: sim.CapabilityModel,
    rest_time: int,
    max_reschedule_weeks: int,
    penalty_next_week: int,
    surgeons: Optional[List[str]] = None,
    n_rooms: Optional[int] = None,
) -> dict:
    """
    Compute metrics compatible with ga_optimize_per_scenario* from a fixed schedule dataframe.

    Expect columns:
      - case_id (or pid), case_type ("URGENT"/"ELECTIVE"), surgery_type
      - arrival (urgent arrival, elective planned_start), start (executed/planned start)
      - room, main, assist1, assist2
    """
    df = schedule_df.copy()
    if "pid" in df.columns and "case_id" not in df.columns:
        df = df.rename(columns={"pid": "case_id"})
    if surgeons is None:
        surgeons = list(work.surgeons)
    if n_rooms is None:
        n_rooms = int(df["room"].max()) if len(df) else 1
    ...
    for _, row in df.iterrows():
        ctype = str(row.get("case_type", row.get("type", ""))).upper()
    ...
    delayed_next_week = int(sum(
        ...
        if str(row.get("case_type", row.get("type", ""))).upper() != "URGENT"
        and int(float(row["start"]) // sim.WEEK_LENGTH) >= 1
    ))
    ...
    for r in log_like:
        ...
        if r.get("assist2"):
            busy_map[str(r["assist2"])].append((int(r["start"]), int(r["end_surgeon"])))
    ...
    return {
        "rooms": int(n_rooms),
        "urgent_arrived": int(sum(1 for _ in df.itertuples() if str(getattr(_, 'case_type', getattr(_, 'type', ''))).upper() == "URGENT")),
        "urgent_started": int(sum(1 for _ in df.itertuples() if str(getattr(_, 'case_type', getattr(_, 'type', ''))).upper() == "URGENT")),
        "urgent_backlog_end": 0,
        "urgent_wait_total": float(urgent_wait_total),
        "urgent_wait_weighted": float(urgent_wait_weighted),
        "elective_started": int(sum(1 for _ in df.itertuples() if str(getattr(_, 'case_type', getattr(_, 'type', ''))).upper() != "URGENT")),
        "elective_delay_total": float(elective_delay_total),
        "elective_delayed_next_week": int(delayed_next_week),
        "overtime_total": int(overtime_total),
        "unproductive_total": int(unproductive_total),
        "elective_delayed_next_week_penalized": int(delayed_next_week * int(penalty_next_week)),
    }
