...
def run_sim(
    ...
    surgeons = [f"S{i}" for i in range(1, 21)]  # S1..S20
    work = WorkSchedule(df_weeks, surgeons)

    cap = load_cap_rank_xlsx(cap_rank_path, capability_sheet='capabilities large')
...
def main():
    ...
    parser.add_argument("--work_schedule", nargs="+", 
        default=[os.path.join(script_dir, "lich_lam_viec_tuan1_large.xlsx"), 
                 os.path.join(script_dir, "lich_lam_viec_tuan2_large.xlsx")], 
        help="Weekly doctor work schedule files (.xlsx), one per week")
    parser.add_argument("--cap_rank", default=os.path.join(script_dir, "Cap_Rank.xlsx"), help="capabilities large + ranking (.xlsx)")
    parser.add_argument("--elective_sched", default=os.path.join(script_dir, "large_scale_result.xlsx"), help="Fixed elective schedule (.xlsx)")
    ...
    if args.mean_urgent is None:
        args.mean_urgent = load_urgent_param_from_excel(args.cap_rank, 'Large scale')
