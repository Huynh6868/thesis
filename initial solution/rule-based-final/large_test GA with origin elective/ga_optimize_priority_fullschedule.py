...
def run_ga_for_scenario(
    ...
    work = sim.WorkSchedule(
        ...
        [f"S{i}" for i in range(1, 21)]  # 20 surgeons for large scale
    )
    cap = sim.load_cap_rank_xlsx(cap_rank_path, capability_sheet='capabilities large')
    ...
    #
    # True-online mode (no lookahead): pass observed_urgent_list from the wrapper.
    # Offline/oracle mode: leave observed_urgent_list=None so we generate the full-week stream from scenario_seed.
    if observed_urgent_list is not None:
        ...
        if ind.fitness is None:
            ...
            if key in cache:
                ...
                ind.fitness, ind.metrics = evaluate_individual(
                    ...
                    max_reschedule_weeks, penalty_next_week, weights, frozen_overrides=frozen_overrides, min_start_time=int(frozen_time) if frozen_time is not None else 0
                )
...
def main():
    ...
    parser.add_argument("--work_schedule", default="lich_lam_viec_tuan1_large.xlsx")
    parser.add_argument("--cap_rank", default="Cap_Rank.xlsx")
    parser.add_argument("--elective_sched", default="large_rulebased_output.xlsx")  # Use rule-based output
    ...
    if args.mean_urgent is None:
        args.mean_urgent = sim.load_urgent_param_from_excel(args.cap_rank, 'Large scale')
    print(f"Using mean_interarrival_urgent: {args.mean_urgent} (Large scale)")
