...
#them vao de validate
parser.add_argument("--validation_mode", choices=["dynamic", "cplex_like"], default="dynamic")
...
class ElectiveCase:
    ...
    baseline_room: int
    baseline_team: Team
    arrival_time: int = 0         # Earliest time this case is allowed to start (e.g., current urgent arrival)
    actual_start: Optional[int] = None  # If known: realized start time from previous simulation/execution
...
def build_valid_teams_by_type(
    cap: sim.CapabilityModel,
    require_assist2: bool = False,
) -> Dict[str, List[Team]]:
    ...
    for stype in cap.main_by_type.keys():
        ...
        # Disable this in CPLEX-like validation because CPLEX requires assist2.
        if not require_assist2:
            for m in mains:
                for a1 in a1s:
                    if a1 == m:
                        continue
...
def build_team_to_idx_map(valid_teams_by_type: Dict[str, List[Team]]) -> Dict[str, Dict[Team, int]]:
    ...
    #dung de tra xem ekip cua lich co dich co idex la bao nhieu
...
def build_elective_plan(
    ...
    max_planning_days: Optional[int] = None,
) -> PlannerOverrides:
    """
    ...
    if max_planning_days is not None:
        time_limit = min(time_limit, max_planning_days * minutes_per_day)
    ...
    # Apply frozen overrides (cases that must NOT be rescheduled, e.g., already started before current urgent arrival)
    frozen_pids: set[int] = set()
    if frozen_overrides is not None:
        frozen_pids = set(frozen_overrides.scheduled_start_by_pid.keys())
        for fpid in frozen_pids:
            if fpid not in elective_cases:
                continue
            fc = elective_cases[fpid]
            fstart = int(frozen_overrides.scheduled_start_by_pid[fpid])
            froom = int(frozen_overrides.room_by_pid[fpid])
            fteam = frozen_overrides.team_by_pid[fpid]
            scheduled_start[fpid] = fstart
            assigned_room[fpid] = froom
            assigned_team[fpid] = fteam

            # Block calendars so rescheduled cases can't overlap in room/surgeons
            main_f, a1_f, a2_f = fteam
            ensure_surg(main_f)
            ensure_surg(a1_f)
            if a2_f:
                ensure_surg(a2_f)

            # Room occupancy includes prep/cleanup
            dur_room_f = fc.duration + fc.prep_time

            # Surgeon occupancy includes rest time
            if isinstance(rest_time, dict):
                val_f = rest_time.get(fc.surgery_type, 30)
                if isinstance(val_f, dict):
                    rt_f = int(max(val_f.values()))
                else:
                    rt_f = int(val_f)
            else:
                rt_f = int(rest_time)
            dur_surg_f = fc.duration + rt_f

            _insert(room_cal[froom], fstart, fstart + dur_room_f)
            _insert(surg_cal[main_f], fstart, fstart + dur_surg_f)
            _insert(surg_cal[a1_f], fstart, fstart + dur_surg_f)
            if a2_f:
                _insert(surg_cal[a2_f], fstart, fstart + dur_surg_f)
    ...
    for pid in priority_list:
        ...
        found = False
        search_start = max(c.planned_start, c.arrival_time, min_start_time)
...
def evaluate_individual(
    ...
    # Step 2: Build priority_rank from permutation (and keep frozen cases deterministic)
    priority_rank = {pid: i for i, pid in enumerate(ind.priority_list)}
    if frozen_overrides is not None:
        frozen_sorted = sorted(
            frozen_overrides.scheduled_start_by_pid.items(),
            key=lambda kv: (int(kv[1]), int(kv[0]))
        )
        for j, (fpid, _) in enumerate(frozen_sorted):
            if fpid not in priority_rank:
                priority_rank[fpid] = j
...
def _get_validation_rest_time(
    rest_time: Union[int, Dict[str, int]],
    surgery_type: str,
) -> int:
    """
    Conservative rest-time extractor for CPLEX-like validation.
    If rest_time is a dict with role-specific values, use the maximum value.
    """
    if isinstance(rest_time, dict):
        val = rest_time.get(surgery_type, 30)
        if isinstance(val, dict):
            return int(max(val.values()))
        return int(val)
    return int(rest_time)


def evaluate_individual_cplex_like(
    ind: GAIndividual,
    elective_cases: Dict[int, ElectiveCase],
    valid_teams_by_type: Dict[str, List[Team]],
    work: sim.WorkSchedule,
    ...
    min_start_time: int = 0,
    big_m: int = 100000,
    violation_penalty: int = 10**9,
) -> Tuple[float, dict]:
    """
    CPLEX-like deterministic validation fitness.

    Objective:
        minimize big_m * Umax + sum_Cmax + violation_penalty * violations

    This approximates the two-phase CPLEX logic:
        Phase 1: minimize maximum surgeon unproductive time.
        Phase 2: minimize total daily OR makespan.

    No urgent patients are considered here.
    """
    ...
    room_intervals: Dict[int, List[Tuple[int, int]]] = {
        r: [] for r in range(1, n_rooms + 1)
    }
    surgeon_intervals: Dict[str, List[Tuple[int, int]]] = {
        s: [] for s in surgeons
    }

    cmax_by_day: Dict[int, int] = {}

    room_overlap = 0
    surgeon_overlap = 0
    qualification_violation = 0
    working_hour_violation = 0
    missing_assignment = 0
    unscheduled_beyond_horizon = 0

    simulation_log: List[Dict] = []

    for pid, case in elective_cases.items():
        if pid not in overrides.scheduled_start_by_pid:
            missing_assignment += 1
            continue

        start = int(overrides.scheduled_start_by_pid[pid])
        room = int(overrides.room_by_pid[pid])
        team = overrides.team_by_pid[pid]

        main, a1, a2 = team

        duration = int(case.duration)
        prep_time = int(case.prep_time)
        rest_val = _get_validation_rest_time(rest_time, case.surgery_type)

        room_end = start + duration + prep_time
        surgeon_end = start + duration + rest_val

        # Check whether the plan failed to schedule this case within horizon.
        if start > time_limit:
            unscheduled_beyond_horizon += 1
            continue

        # Qualification / team-size check.
        valid_teams = valid_teams_by_type.get(case.surgery_type, [])
        if team not in valid_teams:
            qualification_violation += 1

        # CPLEX-like validation requires assist2.
        if a2 is None:
            qualification_violation += 1

        # Working-hour check.
        day_idx = start // minutes_per_day
        weekday = day_idx % 7
        time_in_day = start % minutes_per_day
        if(
            day_idx >= validation_days
            or weekday > 4
            or time_in_day < admin_start
            or time_in_day + duration + prep_time > admin_end
        ):
            working_hour_violation += 1

        surgeons_needed = [main, a1] + ([a2] if a2 else [])
        if not all(work.on_duty_for_elective(s, float(start)) for s in surgeons_needed):
            working_hour_violation += 1
        ...
        if room not in room_intervals:
            ...
            if not _can_insert(room_intervals[room], start, room_end):
                room_overlap += 1
            else:
                _insert(room_intervals[room], start, room_end)

        # Surgeon overlap check.
        for s in surgeons_needed:
            if s not in surgeon_intervals:
                surgeon_intervals[s] = []

            if not _can_insert(surgeon_intervals[s], start, surgeon_end):
                surgeon_overlap += 1
            else:
                _insert(surgeon_intervals[s], start, surgeon_end)

        # Daily OR makespan.
        # CPLEX phase 2 measures Cmax from the beginning of the working horizon.
        # Therefore, subtract admin_start
        # Only count Cmax for the 5 validation days.
        if day_idx < validation_days:
            cmax_in_day = room_end - day_idx * minutes_per_day - admin_start
            cmax_by_day[day_idx] = max(cmax_by_day.get(day_idx, 0), cmax_in_day)

        # Keep same log structure as the dynamic evaluator so the existing export still works.
        simulation_log.append(
            {
                "type": "ELECTIVE",
                "pid": pid,
                "surgery_type": case.surgery_type,
                "planned": case.planned_start,
                "start": start,
                "end": start + duration,
                "end_surgeon": surgeon_end,
                "wait": max(0, start - case.planned_start),
                "room": room,
                "main": main,
                "assist1": a1,
                "assist2": a2,
    ...
    # Compute surgeon unproductive time and Umax.
    duty_map = {
        s: work.duty_intervals_abs(s, time_limit)
        for s in surgeons
    }

    unproductive_by_surgeon: Dict[str, int] = {}

    for s in surgeons:
        duty = sim.merge_intervals(duty_map.get(s, []))
        busy = sim.merge_intervals(surgeon_intervals.get(s, []))

        busy_in_duty = sim.overlap_total(busy, duty)
        duty_total = sim.interval_total(duty)

        unproductive = max(0, duty_total - busy_in_duty)
        unproductive_by_surgeon[s] = int(unproductive)

    Umax = max(unproductive_by_surgeon.values()) if unproductive_by_surgeon else 0
    unproductive_total = sum(unproductive_by_surgeon.values())
    sum_Cmax = sum(cmax_by_day.values())

    total_violations = (
        room_overlap
        + surgeon_overlap
        + qualification_violation
        + working_hour_violation
        + missing_assignment
        + unscheduled_beyond_horizon
    )

    objective = (
        big_m * Umax
        + sum_Cmax
        + violation_penalty * total_violations
    )

    metrics = {
        "objective": float(objective),
        "objective_cplex_like": float(objective),
        "Umax": int(Umax),
        "sum_Cmax": int(sum_Cmax),
        "Cmax_by_day": cmax_by_day,
        "unproductive_total": int(unproductive_total),
        "unproductive_by_surgeon": unproductive_by_surgeon,
        "room_overlap": int(room_overlap),
        "surgeon_overlap": int(surgeon_overlap),
        "qualification_violation": int(qualification_violation),
        "working_hour_violation": int(working_hour_violation),
        "missing_assignment": int(missing_assignment),
        "unscheduled_beyond_horizon": int(unscheduled_beyond_horizon),
...
def run_ga_for_scenario(
    ...
    weights: Dict[str, float],
    seed_ga: int,
    observed_urgent_list: Optional[List[Tuple[float, str]]] = None,
    frozen_time: Optional[int] = None,
    ...
    work = sim.WorkSchedule(
        ...
        [f"S{i}" for i in range(1, 13)]  # 12 surgeons for small scale
    )
    cap = sim.load_cap_rank_xlsx(cap_rank_path)
    
    # Build valid teams
# In CPLEX-like validation, use only 3-person teams to match CPLEX.
require_assist2 = (validation_mode == "cplex_like")
valid_teams = build_valid_teams_by_type(cap, require_assist2=require_assist2)
team_to_idx_map = build_team_to_idx_map(valid_teams)
    
    # Urgent arrivals used for fitness evaluation.
#
# dynamic:
#   - If observed_urgent_list is provided, use it.
#   - Otherwise generate urgent arrivals from scenario_seed.
#
# cplex_like:
#   - No urgent arrivals.
#   - Used for deterministic validation against CPLEX.
if validation_mode == "cplex_like":
    urgent_list = []
    print("\nValidation mode = cplex_like: urgent list is empty.")
elif observed_urgent_list is not None:
    urgent_list = list(observed_urgent_list)
    urgent_list.sort(key=lambda x: x[0])
    print(f"\nUsing observed urgent list (no lookahead): {len(urgent_list)} urgents")
else:
    ...
    n_rooms = int(elective_input_df["room"].max()) if len(elective_input_df) else 2
        ...
        # Build elective cases (two-pass so we can freeze by actual execution time)
    elective_cases: Dict[int, ElectiveCase] = {}
    elective_baseline_data: Dict[int, Dict] = {}
    _row_info: Dict[int, Dict] = {}
    ...
    for _, row in elective_input_df.iterrows():
        ...
        a2 = a2 if a2 else None

        actual_start = None
        if "actual_start" in elective_input_df.columns:
            try:
                if pd.notna(row.get("actual_start")):
                    actual_start = int(row.get("actual_start"))
            except Exception:
                actual_start = None
    ...
    # Determine which elective cases are frozen at current urgent arrival.
    # Rule: frozen iff actual_start < frozen_time (already started); fallback to planned_start if actual_start unknown.
    frozen_pids: set[int] = set()
    if frozen_time is not None:
        ft = int(frozen_time)
        for pid, info in _row_info.items():
            astart = info.get("actual_start")
            if astart is not None:
                if int(astart) < ft:
                    frozen_pids.add(pid)
            else:
                if int(info["planned_start"]) < ft:
                    frozen_pids.add(pid)

    # Build ElectiveCase objects, enforcing constraint: modifiable cases cannot start before frozen_time.
    for pid, info in _row_info.items():
        arrival_time = int(frozen_time) if (frozen_time is not None and pid not in frozen_pids) else 0
        elective_cases[pid] = ElectiveCase(
            pid=pid,
            surgery_type=info["surgery_type"],
            planned_start=int(info["planned_start"]),
            duration=int(info["duration"]),
            prep_time=int(info["prep_time"]),
            baseline_room=int(info["room"]),
            baseline_team=info["team"],
            arrival_time=arrival_time,
            actual_start=info.get("actual_start"),
        )

    # Frozen overrides: lock already-started cases exactly as given (start/room/team).
    frozen_overrides: Optional[PlannerOverrides] = None
    if frozen_time is not None and frozen_pids:
        _fs: Dict[int, int] = {}
        _fr: Dict[int, int] = {}
        _ftm: Dict[int, Team] = {}
        for pid in frozen_pids:
            info = _row_info[pid]
            locked_start = info.get("actual_start")
            if locked_start is None:
                locked_start = info["planned_start"]
            _fs[pid] = int(locked_start)
            _fr[pid] = int(info["room"])
            _ftm[pid] = info["team"]
        frozen_overrides = PlannerOverrides(_fs, _fr, _ftm)

    # Modifiable pids are the decision variables for GA at this trigger
    modifiable_pids = [pid for pid in elective_cases.keys() if pid not in frozen_pids]
# Create baseline individual (identity chromosome)
    baseline_priority = sorted(modifiable_pids, key=lambda p: elective_cases[p].planned_start)
    baseline_team_idx = {}
    ...
    def eval_one(ind: GAIndividual) -> Tuple[float, dict]:
        if validation_mode == "cplex_like":
            return evaluate_individual_cplex_like(
                validation_days = 5,
                ind=ind,
                elective_cases=elective_cases,
                valid_teams_by_type=valid_teams,
                work=work,
                n_rooms=n_rooms,
                rest_time=rest_time,
                max_reschedule_weeks=max_reschedule_weeks,
                frozen_overrides=frozen_overrides,
                min_start_time=int(frozen_time) if frozen_time is not None else 0,
                big_m=100000,
        )
        return evaluate_individual(
        ind=ind,
        elective_cases=elective_cases,
        elective_baseline_data=elective_baseline_data,
        urgent_list=urgent_list,
        valid_teams_by_type=valid_teams,
        work=work,
        cap=cap,
    ...
    baseline_fitness, baseline_metrics = eval_one(baseline_ind)
...
if validation_mode == "cplex_like":
    print(f"  Umax: {baseline_metrics['Umax']}")
    print(f"  Sum Cmax: {baseline_metrics['sum_Cmax']}")
    print(f"  Feasible: {baseline_metrics['feasible']}")
    print(f"  Total violations: {baseline_metrics['total_violations']}")
else:
    ...
    def get_key(ind: GAIndividual) -> str:
        ...
        #IMPORTANT: must include ALL chromosome components that affect the decoded plan.
        parts = [",".join(map(str, ind.priority_list))]
        parts.append("|".join(f"{pid}:{idx}" for pid, idx in sorted(ind.team_idx_by_pid.items())))
        if ind.room_by_pid:
            parts.append("|".join(f"{pid}:{r}" for pid, r in sorted(ind.room_by_pid.items())))
        return "#".join(parts)
    ...
    for ind in pop:
        if ind.fitness is None:
            ...
            if key in cache:
                ...
                ind.fitness, ind.metrics = eval_one(ind)
    ...
    for gen in range(1, gens + 1):
        ...
        while len(new_pop) < pop_size:
            ...
            if rng.random() < cx_rate:
                ...
                # Uniform crossover for room indices (guard against None)
                if c1.room_by_pid and c2.room_by_pid:
...
def main():
    ...
    parser.add_argument("--work_schedule", default="lich_lam_viec_tuan1.xlsx")
    parser.add_argument("--cap_rank", default="Cap_Rank.xlsx")
    parser.add_argument("--elective_sched", default="small_rulebased_output.xlsx")  # Use rule-based output
    ...
    parser.add_argument(
    "--validation_mode",
    choices=["dynamic", "cplex_like"],
    default="dynamic",
    help="Use 'cplex_like' to disable urgent arrivals for deterministic validation."
)
    
    args = parser.parse_args()
    # Load urgent parameter from Excel based on scale
    if args.mean_urgent is None:
        args.mean_urgent = sim.load_urgent_param_from_excel(args.cap_rank, 'Small scale')
...
if args.validation_mode == "cplex_like":
    payload["comparison_metrics"] = {
        "baseline_Umax": baseline_metrics.get("Umax"),
        "ga_Umax": best_metrics.get("Umax"),
        "baseline_sum_Cmax": baseline_metrics.get("sum_Cmax"),
        "ga_sum_Cmax": best_metrics.get("sum_Cmax"),
        "baseline_objective_cplex_like": baseline_metrics.get("objective_cplex_like"),
        "ga_objective_cplex_like": best_metrics.get("objective_cplex_like"),
        "baseline_feasible": baseline_metrics.get("feasible"),
        "ga_feasible": best_metrics.get("feasible"),
        "ga_total_violations": best_metrics.get("total_violations"),
    }

    if baseline_metrics.get("objective_cplex_like", 0) > 0:
        payload["improvement"] = {
            "objective_cplex_like": (
                baseline_metrics["objective_cplex_like"]
                - best_metrics["objective_cplex_like"]
            ),
            "improvement_pct": (
                (
                    baseline_metrics["objective_cplex_like"]
        ...
        payload["improvement"] = {
            "objective_cplex_like": None,
            "improvement_pct": None,
        }

else:
    payload["improvement"] = {
        "urgent_wait_weighted": (
            baseline_metrics["urgent_wait_weighted"]
            - best_metrics["urgent_wait_weighted"]
        ),
        "elective_delay_total": (
            baseline_metrics["elective_delay_total"]
            - best_metrics["elective_delay_total"]
        ),
        "overtime_total": (
            baseline_metrics["overtime_total"]
            - best_metrics["overtime_total"]
        ),
    }
