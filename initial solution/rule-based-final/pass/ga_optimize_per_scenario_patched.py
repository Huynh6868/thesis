...
def simulate_fixed_urgent(
        ...
        while True:
            ...
            for c in waiting_e:
                ...
                if not sim.is_admin_time(now_int):
                    new_start = sim.find_earliest_elective_slot(now_int, c, elective_cases, work, rest_time, max_reschedule_weeks)
    ...
    time_limit = (1 + max_reschedule_weeks) * sim.WEEK_LENGTH
    duty_map = {s: work.duty_intervals_minutes(s, time_limit) for s in surgeons}
...
def simulate_fixed_urgent_lightweight(
    ...
    """
    work = sim.load_work_schedule_xlsx(work_schedule_path)
    cap = sim.load_cap_rank_xlsx(cap_rank_path)
...
def main():
    ...
    ap.add_argument("--max_weeks", type=int, default=sim.DEFAULT_MAX_RESCHEDULE_WEEKS)
    ...
    base_df = sim.load_elective_schedule_xlsx(args.elective_sched)
