...
import lightweight_fitness_v3 as lf

# Use lightweight evaluator for GA fitness (SimPy only for final validation)
USE_LIGHTWEIGHT_FOR_FITNESS = True
...
def simulate_fixed_urgent_lightweight(
    ...
    """
    Drop-in replacement for simulate_fixed_urgent() that avoids SimPy.

    NOTE: seed is unused (deterministic given elective_df and urgent_list),
    ...
    work = sim.WorkSchedule(df_work, surgeons)
    cap = sim.load_cap_rank_xlsx(cap_rank_path)
    return lf.simulate_fixed_urgent_lightweight(
        work=work,
        cap=cap,
        elective_df=elective_df,
...
def get_rule_based_baseline(
    ...
    """
    Run rule-based simulation with CPLEX schedule + urgent arrivals,
    then extract the ACTUAL scheduled elective cases (after rescheduling).
    
    Returns: (rule_based_schedule_df, baseline_metrics)
    """
    # Run full simulation to get metrics AND scheduled cases
    import simpy
    ...
    # Build elective cases from CPLEX
    cplex_df = cplex_df.copy()
    cplex_df["pid"] = cplex_df["pid"].astype(str)
    
    elective_cases: Dict[str, sim.ElectiveCase] = {}
    for _, row in cplex_df.iterrows():
        pid = str(row["pid"])
        stype = str(row["surgery_type"])
        day = int(row["day"])
        hhmm = str(row["time_hhmm"])
    ...
    # Run simulation (same logic as simulate_fixed_urgent)
    urgent_cases: List[sim.UrgentCase] = []
    urgent_stream = sorted([(float(t), str(st)) for t, st in urgent_list], key=lambda x: x[0])
    
    n_rooms = int(max(cplex_df["room"].max(), 2))
    env = simpy.Environment()
    room_res = {r: simpy.Resource(env, capacity=1) for r in range(1, n_rooms + 1)}
    surg_res = {s: simpy.Resource(env, capacity=1) for s in surgeons}
    rooms = list(room_res.keys())
    ...
    # Urgent generator + scheduler (abbreviated - same as simulate_fixed_urgent)
    def urgent_generator_fixed():
        nonlocal urgent_cases
        for (t, stype) in urgent_stream:
            if t < env.now:
    ...
    def scheduler():
        # Same scheduler logic as simulate_fixed_urgent
        # (Abbreviated for brevity - uses same reschedule logic)
        arrival_horizon = sim.WEEK_LENGTH
        time_limit = (1 + max_reschedule_weeks) * sim.WEEK_LENGTH
        ...
        while True:
            ...
            # Urgent dispatch (simplified - follow same logic)
            available_urgent_surgeons = {s for s in free_surgeons if work.on_duty_for_urgent(s, now)}
            waiting_urgent_sorted = sorted(
                [u for u in urgent_cases if u.status == "waiting" and u.arrival_time <= now],
                key=lambda u: (u.arrival_bucket, u.rank, u.arrival_time, u.uid),
            ...
            for u in waiting_urgent_sorted:
                ...
                stats["urgent_started"] += 1
                env.process(sim.execute_surgery(
                    env=env, pid=u.uid, surgery_type=u.surgery_type, is_urgent=True,
                    room=room, team=(main, a1, a2), room_res=room_res, surg_res=surg_res,
                    rest_time=rest_time, log=log, active_counter=active_counter,
                    arrival_time=u.arrival_time, start_time=now, case_obj=u,
                ))
            
            # Elective dispatch with rescheduling
            waiting_e = [c for c in elective_cases.values() if c.status == "waiting" and c.scheduled_start <= now_int]
            waiting_e.sort(key=lambda c: (c.scheduled_start, c.pid))
            ...
            for c in waiting_e:
                ...
                env.process(sim.execute_surgery(
                    env=env, pid=c.pid, surgery_type=c.surgery_type, is_urgent=False,
                    room=c.room, team=(c.main, c.assist1, c.assist2 if c.assist2 else None),
                    room_res=room_res, surg_res=surg_res, rest_time=rest_time,
                    log=log, active_counter=active_counter,
                    arrival_time=float(c.planned_start), start_time=float(now_int), case_obj=c,
                ))
    ...
    # Calculate metrics (same as simulate_fixed_urgent)
    urgent_recs = [r for r in log if r["type"] == "URGENT"]
    elective_recs = [r for r in log if r["type"] == "ELECTIVE"]
    ...
    # Extract scheduled elective cases to DataFrame
    rule_based_rows = []
    for c in elective_cases.values():
        day = c.scheduled_start // sim.MINUTES_PER_DAY
        time_in_day = c.scheduled_start % sim.MINUTES_PER_DAY
        rule_based_rows.append({
            "pid": c.pid,
            "surgery_type": c.surgery_type,
            "day": int(day),
            "time_hhmm": minutes_to_hhmm_in_day(time_in_day),
            "room": c.room,
            "main": c.main,
            "assist1": c.assist1,
            "assist2": c.assist2 if c.assist2 else "",
        })
    
    rule_based_df = pd.DataFrame(rule_based_rows)
    rule_based_df = rule_based_df[["pid", "surgery_type", "day", "time_hhmm", "room", "main", "assist1", "assist2"]]
    
    return rule_based_df, baseline_metrics
...
def run_ga_for_scenario(
    ...
    # NEW APPROACH: Get rule-based baseline (CPLEX + urgent rescheduling)
    print("\n" + "="*60)
    print("GENERATING RULE-BASED BASELINE...")
    print("="*60)
    rule_based_df, baseline_metrics = get_rule_based_baseline(
        work_schedule_path=work_schedule_path,
        cap_rank_path=cap_rank_path,
        cplex_df=base_df,  # Input: CPLEX schedule
        urgent_list=urgent_list,
        rest_time=rest_time,
        max_reschedule_weeks=max_reschedule_weeks,
        penalty_next_week=penalty_next_week,
    ...
    print(f"Rule-based baseline metrics: elective_delay={baseline_metrics['elective_delay_total']:.0f}, overtime={baseline_metrics['overtime_total']}")
    
    # GA will now optimize from rule-based schedule (not CPLEX)
    # Rebuild day_cases from rule-based schedule
    rule_based_df = rule_based_df.copy()
    rule_based_df["pid"] = rule_based_df["pid"].astype(str)
    day_cases = {d: rule_based_df[rule_based_df["day"] == d]["pid"].tolist() for d in sorted(rule_based_df["day"].unique())}
    
    # baseline planned start (from rule-based, not CPLEX) for shift penalty
    base_planned_abs = {
        str(r.pid): int(r.day) * sim.MINUTES_PER_DAY + sim.hhmm_to_minutes(str(r.time_hhmm))
        for r in rule_based_df.itertuples(index=False)
    }
    
    # build baseline individual from rule-based schedule
    base_perms = {d: day_cases[d][:] for d in day_cases.keys()}
    base_room = {pid: int(rule_based_df.set_index("pid").loc[pid, "room"]) for pid in rule_based_df["pid"].tolist()}
    ...
    def evaluate(ind: GAIndividual) -> Tuple[float, dict, pd.DataFrame]:
        sched_df = decode_to_schedule_df(rule_based_df, ind, K=K, rest_time=rest_time, allow_after_admin=False)
        ...
        m = (simulate_fixed_urgent_lightweight if USE_LIGHTWEIGHT_FOR_FITNESS else simulate_fixed_urgent)(
            work_schedule_path=work_schedule_path,
            cap_rank_path=cap_rank_path,
    ...
    # generations
    log_file = f"ga_progress_seed{scenario_seed}.log"
    with open(log_file, "w") as f:
        f.write(f"Starting GA with {pop_size} individuals for {gens} generations...\n")
    print(f"\nStarting GA with {pop_size} individuals for {gens} generations...")
    ...
    for g in range(1, gens + 1):
        ...
        new_pop.extend([clone_ind(pop[0]), clone_ind(pop[1])])
        
        # Print and log progress every generation (for monitoring)
        best_fitness = pop[0].fitness if pop[0].fitness is not None else float("inf")
        best_elec_delay = pop[0].metrics.get("elective_delay_total", 0) if pop[0].metrics else 0
        
        # Write to log file every generation
        with open(log_file, "a") as f:
            f.write(f"Gen {g}/{gens}: Best fitness = {best_fitness:.1f}, Elective delay = {best_elec_delay:.0f}\n")
        
        # Print to console every 10 generations
        if g % 10 == 0 or g == 1:
            print(f"Gen {g}/{gens}: Best fitness = {best_fitness:.1f}, Elective delay = {best_elec_delay:.0f}", flush=True)
    ...
    # VALIDATE final best solution with FULL SimPy simulation (if using lightweight)
    if USE_LIGHTWEIGHT_FOR_FITNESS:
        print("\n" + "="*60)
        ...
        if best_simpy_metrics['elective_delay_total'] > 0:
            elec_error = abs(lw_metrics['elective_delay_total'] - best_simpy_metrics['elective_delay_total']) / best_simpy_metrics['elective_delay_total'] * 100
            print(f"  Elective delay error: {elec_error:.1f}%")
        if best_simpy_metrics['overtime_total'] > 0:
            ot_error = abs(lw_metrics['overtime_total'] - best_simpy_metrics['overtime_total']) / best_simpy_metrics['overtime_total'] * 100
            print(f"  Overtime error: {ot_error:.1f}%")
        
        # Use SimPy metrics for final reporting (more accurate)
        return best_sched_df, best_simpy_metrics, baseline_metrics
    else:
        return best_sched_df, best_ind.metrics, baseline_metrics
...
def main():
    ...
    ap.add_argument("--max_weeks", type=int, default=0)  # No longer used in v3
