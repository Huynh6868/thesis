...
# Short name aliases for surgery types (used in Excel)
SURGERY_TYPE_ALIASES = {
    # Full names
    "adenotonsillectomy": "adenotonsillectomy",
    "microlaryngoscopy": "microlaryngoscopy",
    "septoplasty": "septoplasty",
    "thyroidectomy": "thyroidectomy",
    "buccal mucosa bioppsy": "buccal mucosa bioppsy",
    "excision of the lymphadenopathy from the lumbar": "excision of the lymphadenopathy from the lumbar",
    "modified radical mastoidectomy": "modified radical mastoidectomy",
    "rhinoplasty": "rhinoplasty",
    "endoscopic sinus": "endoscopic sinus",
    "sleep apnea diagnosis test": "sleep apnea diagnosis test",
    ...
    "mastoid": "modified radical mastoidectomy",
    "mastoidectomy": "modified radical mastoidectomy",
    "modified": "modified radical mastoidectomy",
    "rhino": "rhinoplasty",
    "sinus": "endoscopic sinus",
    "endos": "endoscopic sinus",
    "endoscopic": "endoscopic sinus",
    "sleep": "sleep apnea diagnosis test",
    "sleep apnea": "sleep apnea diagnosis test",
}

def normalize_surgery_type(stype: str) -> str:
    """Normalize surgery type name to full canonical name."""
    stype_lower = stype.strip().lower()
    return SURGERY_TYPE_ALIASES.get(stype_lower, stype)
...
def load_cap_rank_xlsx(path: str, capability_sheet: str = "Capabilities") -> CapabilityModel:
    """
    Load capability and ranking data from Cap_Rank.xlsx
    
    Args:
        path: Path to Cap_Rank.xlsx file
        capability_sheet: Name of the capabilities sheet to load (e.g., 'Capabilities', 'capabilities med', 'capabilities large')
    """
    df_cap = pd.read_excel(path, sheet_name=capability_sheet)
...
def load_elective_schedule_xlsx(path: str, return_df: bool = False):
    ...
    for _, row in df.iterrows():
        ...
        stype = normalize_surgery_type(str(row["surgery_type"]))
        day = int(row["day"])
...
def elective_overlaps(case_a: ElectiveCase, case_b: ElectiveCase, rest_time_map: Dict[str, Dict[str, int]]) -> bool:
    ...
    Rest time is determined by the surgeon's role in each case (main vs assistant).
    """
    ...
    # surgeon overlap - check each shared surgeon with role-specific rest time
    team_a = {case_a.main, case_a.assist1, case_a.assist2}
    team_b = {case_b.main, case_b.assist1, case_b.assist2}
    shared_surgeons = team_a & team_b
    
    if shared_surgeons:
        for surgeon in shared_surgeons:
            if surgeon is None:  # Skip None assistants
                continue
                
            # Determine role of this surgeon in case_a
            if surgeon == case_a.main:
                a_rest = rest_time_map.get(case_a.surgery_type, {}).get('main', 15)
            else:  # assistant1 or assistant2
                a_rest = rest_time_map.get(case_a.surgery_type, {}).get('assistant', 15)
            
            # Determine role of this surgeon in case_b
            if surgeon == case_b.main:
                b_rest = rest_time_map.get(case_b.surgery_type, {}).get('main', 15)
            else:  # assistant1 or assistant2
                b_rest = rest_time_map.get(case_b.surgery_type, {}).get('assistant', 15)
            
            # Calculate end times with role-specific rest times
            a_surg_end = a_start + case_a.duration() + a_rest
            b_surg_end = b_start + case_b.duration() + b_rest
            
            # Check overlap for this surgeon
...
def is_elective_slot_feasible(
    ...
    # Use MAX rest time to ensure ALL team members (main + assistants) finish within admin time
    main_rest = rest_time_map.get(case.surgery_type, {}).get('main', 15)
    asst_rest = rest_time_map.get(case.surgery_type, {}).get('assistant', 15)
    rest_time = max(main_rest, asst_rest)
...
def find_earliest_elective_slot(
    ...
    # Get MAX rest time to ensure ALL team members finish within admin time
    main_rest = rest_time_map.get(case.surgery_type, {}).get('main', 15)
    asst_rest = rest_time_map.get(case.surgery_type, {}).get('assistant', 15)
    rest_time = max(main_rest, asst_rest)
...
def execute_surgery(
    ...
    #Holds room and surgeons with role-specific rest times:
    # - room for duration+prep
    # - main surgeon for duration+main_rest
    # - assistants for duration+asst_rest
    duration = SURGERY_DURATION_MIN[surgery_type]
    prep = PREP_TIME_MIN[surgery_type]

    main, a1, a2 = team
    
    # Get role-specific rest times
    main_rest = rest_time_map.get(surgery_type, {}).get('main', 15)
    asst_rest = rest_time_map.get(surgery_type, {}).get('assistant', 15)
    ...
    # Calculate end times for each resource type
    end_main = start_time + duration + main_rest
    end_asst = start_time + duration + asst_rest
    ...
    # Release resources in order of completion (role-specific timelines)
    release_events = [
        (main_rest, 'main'),
        (asst_rest, 'asst'),
        (prep, 'room'),
    ...
    current_time = 0
    for wait_time, resource_type in release_events:
        if wait_time > current_time:
            yield env.timeout(wait_time - current_time)
            current_time = wait_time
        
        if resource_type == 'main':
            surg_res[main].release(req_main)
        elif resource_type == 'asst':
...
def run_sim(
    ...
    surgeons = [f"S{i}" for i in range(1, 17)]  # S1..S16
    work = WorkSchedule(df_weeks, surgeons)

    cap = load_cap_rank_xlsx(cap_rank_path, capability_sheet='capabilities med')
    ...
    def delay_to_next_week(case: ElectiveCase, now_int: int, reason: str) -> None:
        """Push an elective case to next week's first admin slot (Mon 08:00).

        This keeps the case in the system (status remains 'scheduled') so it can be dispatched
        in week+1 if the simulation horizon allows.
        """
        old = case.scheduled_start
        current_week = now_int // WEEK_LENGTH
        next_week_start = (current_week + 1) * WEEK_LENGTH
        next_start = next_week_start + ADMIN_SHIFT_START  # Monday 08:00
        case.scheduled_start = int(next_start)
        case.delayed_weeks = max(case.delayed_weeks, current_week + 1)
        # Track stats (unique delayed cases are computed later; this is a log of events)
        stats["elective_delayed_next_week"] += 1
        stats["elective_rescheduled"].append({"pid": case.pid, "from": old, "to": next_start, "reason": reason})
            ...
            for c in due_electives:
                ...
                if not is_admin_time(now_int):
                    ...
                    if new_start is None:
                        delay_to_next_week(c, now_int, "Not admin time - no slot in current week")
                        continue
                ...
                if not (work.on_duty_for_elective(c.main, now) and work.on_duty_for_elective(c.assist1, now) and work.on_duty_for_elective(c.assist2, now)):
                    new_start = find_earliest_elective_slot(now_int, c, elective_cases, work, rest_time_map)
                    if new_start is None:
                        delay_to_next_week(c, now_int, "Surgeon day off - no slot in current week")
                        continue
                    old = c.scheduled_start
                    c.scheduled_start = new_start
                    c.delayed_weeks = max(c.delayed_weeks, new_start // WEEK_LENGTH)
                ...
                if c.room not in free_rooms:
                    ...
                    new_start = find_earliest_elective_slot(now_int, c, elective_cases, work, rest_time_map)
                    if new_start is None:
                        delay_to_next_week(c, now_int, "Room busy - no slot in current week")
                        continue
                    old = c.scheduled_start
                    c.scheduled_start = new_start
                    c.delayed_weeks = max(c.delayed_weeks, new_start // WEEK_LENGTH)
                ...
                if not needed.issubset(free_surgeons):
                    new_start = find_earliest_elective_slot(now_int, c, elective_cases, work, rest_time_map)
                    if new_start is None:
                        delay_to_next_week(c, now_int, "Surgeon busy - no slot in current week")
                        continue
                    old = c.scheduled_start
                    c.scheduled_start = new_start
                    c.delayed_weeks = max(c.delayed_weeks, new_start // WEEK_LENGTH)
                ...
                if (not is_admin_time(end_surg - 1)) or (not same_calendar_day(now, end_surg - 1)):
                    new_start = find_earliest_elective_slot(now_int, c, elective_cases, work, rest_time_map)
                    if new_start is None:
                        delay_to_next_week(c, now_int, "Would exceed admin hours - no slot in current week")
                        continue
                    old = c.scheduled_start
                    c.scheduled_start = new_start
                    c.delayed_weeks = max(c.delayed_weeks, new_start // WEEK_LENGTH)
    ...
    sim_end = 0
    for rec in log:
        sim_end = max(sim_end, int(math.ceil(rec["end_room"])), int(math.ceil(max(rec["end_main"], rec["end_asst"]))))
    ...
    for rec in log:
        ...
        b = int(math.ceil(max(rec["end_main"], rec["end_asst"])))
...
def main():
    ...
    parser.add_argument("--work_schedule", nargs="+", 
        default=[os.path.join(script_dir, "lich_lam_viec_tuan1_med.xlsx"), 
                 os.path.join(script_dir, "lich_lam_viec_tuan2_med.xlsx")], 
        help="Weekly doctor work schedule files (.xlsx), one per week")
    parser.add_argument("--cap_rank", default=os.path.join(script_dir, "Cap_Rank.xlsx"), help="capabilities med + ranking (.xlsx)")
    parser.add_argument("--elective_sched", default=os.path.join(script_dir, "medium_scale_result.xlsx"), help="Fixed elective schedule (.xlsx)")
    parser.add_argument("--mean_urgent", type=float, default=None, help="Mean inter-arrival time for urgent (minutes)")
    parser.add_argument("--rest_time", type=int, default=None, help="(Deprecated) Surgeon rest time - now loaded from Excel")
    ...
    if args.mean_urgent is None:
        args.mean_urgent = load_urgent_param_from_excel(args.cap_rank, 'Medium scale')
    print(f"Using mean_interarrival_urgent: {args.mean_urgent} (Medium scale)")
    
    # Load rest time map from Excel (args.rest_time is kept for backward compatibility but not used)
    rest_time_map = load_rest_time_map(args.cap_rank)
    ...
    default_output = os.path.join(script_dir, "medium_rulebased_output.xlsx")
