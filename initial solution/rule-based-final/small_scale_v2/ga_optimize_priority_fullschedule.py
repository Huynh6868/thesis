...
def _can_insert(intervals: List[Tuple[int, int]], start: int, end: int) -> bool:
    """
    Check if [start, end) can be inserted without overlap. -> Ex: chen them 1 ca vao giua 2 ca khac
    thi co duoc khong, co bi overlap khong
    """
    if not intervals:
        return True
...
def build_elective_plan(
    ...
    for pid in priority_list:
        ...
        if room_by_pid is not None and pid in room_by_pid:
            ...
            # If no room constraint, try all rooms 
...
def run_ga_for_scenario(
    ...
    work = sim.WorkSchedule(
        [sim.load_work_schedule_xlsx(work_schedule_path)],  # Wrapped in list for multi-week support
    ...
    if not modifiable_pids:
        ...
        # Build output DataFrame with frozen cases
        schedule_rows = []
...
def main():
    ...
    parser.add_argument("--work_schedule", default=os.path.join(_SCRIPT_DIR, "lich_lam_viec_tuan1.xlsx"))
    parser.add_argument("--cap_rank", default=os.path.join(_SCRIPT_DIR, "Cap_Rank.xlsx"))
    parser.add_argument("--elective_sched", default=os.path.join(_SCRIPT_DIR, "small_rulebased_output.xlsx"))  # Use rule-based output
