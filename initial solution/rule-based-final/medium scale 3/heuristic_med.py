...
def solve_heuristic_from_file(dat_file):
    ...
    # --- REPORT ---
    print(f"\nMEDIUM SCALE RESULTS (100 patients):")
    print(f"- Successfully scheduled: {len(assignments)}/{len(P)} ({len(assignments)/len(P)*100:.1f}%)")
...
if __name__ == "__main__":
    # Đảm bảo bạn đã chạy file data med 80.py trước để có file .dat
    try:
        solve_heuristic_from_file("medium_scale_100p.dat")
    except FileNotFoundError:
        print("ERROR: File 'medium_scale_100p.dat' not found.")
        print("Please run 'data med 100.py' first to generate data!")