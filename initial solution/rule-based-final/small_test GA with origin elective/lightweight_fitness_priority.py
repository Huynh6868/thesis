...
def simulate_elective_plan_with_urgent(
    ...
    # Simulation time
    now = 0
    # Reduced buffer: only need enough time to handle late urgent arrivals
    # Most urgents arrive within time_limit, add 1-2 days buffer is sufficient
    max_time = time_limit + 2 * 24 * 60  # 2 days buffer instead of 7 days
    ...
    while now < max_time:
        ...
        if not ready_urgent and not due_elective:
            ...
            # Early termination: if all urgents completed and no elective can start, stop
            if not waiting_urgent_future and not waiting_elective:
                break
            
            next_arr = min((_ceil_minute(u.arrival_time) for u in waiting_urgent_future), default=None)
            # Only consider elective scheduled_start within time_limit to avoid jumping too far
            next_elec = min((e.scheduled_start for e in waiting_elective 
                           if e.scheduled_start > now and e.scheduled_start <= time_limit), default=None)
            ...
            if in_admin_shift:
                for e in eligible:
                    ...
                    if time_in_day + e.duration + e.prep_time > sim.ADMIN_SHIFT_END:
                        # Don't mark as delayed yet - might be able to dispatch on a later day
                        # Only skip this attempt for today
                        continue
    ...
    # Final check: Mark any remaining waiting electives as delayed
    # (cases that couldn't be dispatched within time_limit)
    for e in elective_cases.values():
        if e.status == "waiting":
            e.delayed_weeks += 1
