...
ADMIN_SHIFT_START = 8 * 60 
ADMIN_SHIFT_END = 16 * 60

# Parameters
DEFAULT_MEAN_INTERARRIVAL_URGENT = 2520  # Can be overridden by Excel
DEFAULT_REST_TIME_MIN = 15               # Can be overridden by dict from Excel
DEFAULT_MAX_RESCHEDULE_WEEKS = 4 
DEFAULT_PENALTY_DELAY_NEXT_WEEK = 10000 
...
class WorkSchedule:
    ...
    def duty_intervals_abs(self, surgeon: str, sim_end: int) -> List[Tuple[int,int]]:
        #Build absolute duty intervals for surgeon, repeating weeklyweekly.
...
def execute_surgery(
    ...
    # Create list of (time, resource_type, release_action)
    release_events = [
        (main_rest, 'main'),
        (asst_rest, 'asst'),
        (prep, 'room'),
